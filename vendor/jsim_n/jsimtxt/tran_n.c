...
int
step_control(current_step, new_step, hptr)

double current_step, *new_step;
double *hptr;
{
...
int
nonlinear_loop(source_only, hptr)
...
void
init_breakpoint(stime, ftime, step)

double stime, ftime, step;
{
...
void
add_breakpoint(stime, ftime, step)
...

{
  ...
  { 
    ...
    else current_bkpt = current_bkpt->right;
    */

	/* SRW */
    if (temp_stime < current_bkpt->stime) {
	  if (current_bkpt->left != NULL) current_bkpt = current_bkpt->left;
	  else break;
	}
	else {
	  if (current_bkpt->right != NULL) current_bkpt = current_bkpt->right;
	  else break;
	}
  }
...
int
matrix_iteration_update(source_only, hptr)
...
int
matrix_nonlin_iteration_update(source_only, hptr)
...
double
get_pr_val(inc_rate, which_eqn, which_val)
