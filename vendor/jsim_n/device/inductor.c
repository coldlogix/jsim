...
device *
ind_to_devlist(name, type, nodeptr, intptr, dataptr)
...
device *
ind_to_deftree(dev_name, type, nodeptr, intptr, dataptr)
...
device *
indsub_to_devlist(dev_name, type, data)
...
device *
add_ind(name, val, ic, n_plus, n_minus, branch, fcheck, fvalue)

long name;
double val, ic;
long n_plus, n_minus, branch;
int fcheck;
double fvalue;
{
  char *data;
...
int
ind_apriori_step_limit(step, newstep)
...
void
ind_tran_print(fp, inc_rate, prtype, temp_dev)
