...
device *
jj_to_devlist(name, type, nodeptr, intptr, dataptr, modptr)
...
device *
jj_to_deftree(dev_name, type, nodeptr, intptr, dataptr, modptr)
...
device *
jjsub_to_devlist(dev_name, type, data)
...
device *
add_jj(name, area, v_ic, phi_ic, n_plus, n_minus, con_dev, mod)

long name;
double area, v_ic, phi_ic;
long n_plus, n_minus;
char *con_dev;
modeldata  *mod;
{
...
void
jj_quasi(temp_jj, val_quasi, which_piece, gval_quasi, sval_quasi)
...
int
jj_apriori_step_limit(step, newstep)
...
void
jj_tran_print(fp, inc_rate, prtype, subtype, temp_dev)
