...

{
  ...
  if (current_sub_def == NULL)
  {
    ...
    mut_to_devlist(name, MUTUAL_L, (int *) primary_name, 
                   (long *) second_name, dataptr);
  }
  else
    mut_to_deftree(dev_name, MUTUAL_L, (int *) primary_name, 
                   (long *) second_name, dataptr);
...
device *
mut_to_devlist(name, type, nodeptr, intptr, dataptr)
...
device *
mut_to_deftree(dev_name, type, nodeptr, intptr, dataptr)
...
device *
mutsub_to_devlist(dev_name, type, data)
...
device *
add_mut(name, val, primary_name, second_name)
