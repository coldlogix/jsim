...
read_pjj()
{
  ...
  *(dataptr + 2) = 0.0;
  ...
  if (read_error != OK)
  {
    printf("## Error -- bad pjj definition\n");
    no_go = TRUE;
    bad_dev = TRUE;
  }
  ...
  if (bad_dev == FALSE)
  {
    if (current_sub_def == NULL)
    {
      ...
      pjj_to_devlist(name, P_JJ, nodeptr, intptr, dataptr, modptr);
    }
    else
      pjj_to_deftree(dev_name, P_JJ, nodeptr, intptr, dataptr, modptr);
  }
...
device *
pjj_to_devlist(long name, int type, long *nodeptr, long *intptr,
    double *dataptr, modeldata *modptr)
{
  ...
  switch (type)
  {
    case P_JJ : 
         ...
         return(add_pjj(name, *dataptr, *(dataptr+1), *(dataptr+2), 
...
device *
pjj_to_deftree(char *dev_name, int type, long *nodeptr, long *intptr,
    double *dataptr, modeldata *modptr)
{
  long name;
...
device *
pjjsub_to_devlist(char *dev_name, int type, char *data)
{
  int n_plus, n_minus; 
  long name;
  char *con_dev;
  ...
  switch (type)
  {
    case P_JJ : 
         ...
         bmarker = add_bmarker();
         n_plus = getnode_subnode(((sub_pjj *) data)->n_plus,
                                  ((sub_pjj *) data)->p_node);
         n_plus = add_node(n_plus, name, type, PLUS_NODE, bmarker);
         n_minus = getnode_subnode(((sub_pjj *) data)->n_minus,
                                   ((sub_pjj *) data)->m_node);
         n_minus = add_node(n_minus, name, type, MINUS_NODE,
                            bmarker);

         if (((sub_pjj *) data)->con_dev != NULL)
           con_dev = get_subdev_name(((sub_pjj *) data)->con_dev);
         else con_dev = NULL;

         return(add_pjj(name, ((sub_pjj *) data)->area, 
                       ((sub_pjj *) data)->v_ic, 
                       ((sub_pjj *) data)->phi_ic, 
  ...
  while (temp_dev != NULL)
  {
    ...
    else
    {
      ...
      printf("## Error -- model %s for %s does not exit\n",
              (char *) temp_pjj->mod, temp_name->real_name);
      temp_pjj->mod = NULL;
    } 
...
device *
add_pjj(long name, double area, double v_ic, double phi_ic, long n_plus,
    long n_minus, char *con_dev, modeldata *mod)
{
  ...
  if (current_sub_def == NULL)
  {
    temp = (dev_pjj *) mycalloc(1, sizeof(dev_pjj));
    ...
    data = (char *) temp;  
    return(add_dev(name, P_JJ, data));
  }
  else 
  {
    temp_sub = (sub_pjj *) mycalloc(1, sizeof(sub_pjj));
    ...
    data = (char *) temp_sub;  
    return(add_sub((char *) name, P_JJ, data));
  }
...
pjj_dependent()
{
  ...
  while (temp_dev != NULL)
  {
    ...
    {
      type = get_devname_type(temp_pjj->con_dev);
      ...
      switch (type)
      {
        ...
        case V_SOURCE : 
             ...
             con_dev = find_dev(temp_pjj->con_dev, type);
             if (con_dev == NULL)
             {
               temp_name = search_name(temp_dev->name);
               ...
               printf(" %s not found\n", temp_name->real_name); 
               warned = TRUE;
               free(temp_pjj->con_dev);
               temp_pjj->con_dev = NULL;
             }
             else
               temp_pjj->con_dev = (char *) con_dev;
             break;
...
pjj_matrix()
{
  ...
  while (temp_dev != NULL)
  {
    temp = (dev_pjj *) temp_dev->data;
    temp_mod = (mod_pjj *) temp->mod->data;
...
double
pjj_condev_i(dev_pjj *temp_pjj, double time)
{
  device *temp_dev;

  temp_dev = (device *) temp_pjj->con_dev;
...
void
pjj_quasi(dev_pjj *temp_pjj, double val_quasi, int *which_piece,
    double *gval_quasi, double *sval_quasi)
{
  mod_jj *temp_mod;

  temp_mod = (mod_pjj *) (temp_pjj->mod->data);
  ...
  {
    if ( temp_pjj->which_piece == SUB_GAP)
    {
      *which_piece = SUB_GAP;
      *gval_quasi = temp_pjj->gg;
      *sval_quasi = 0.0;
    ...
    else
    {
      ...
      *gval_quasi = temp_pjj->glarge;
      *sval_quasi = val_quasi*(temp_pjj->gg - temp_pjj->glarge);
      return;
    }
  }
  ...
  if (val_quasi <= temp_mod->quasi_bkpoint[2])
  {
    *which_piece = TRANSITION;
    *gval_quasi = temp_pjj->glarge;
    *sval_quasi = temp_mod->vgap*(temp_pjj->gg - temp_pjj->glarge);
    return;
  }

  if (val_quasi <= temp_mod->quasi_bkpoint[3])
  {
    if (temp_pjj->which_piece == NORMAL)
    {
      *which_piece = NORMAL;
      *gval_quasi = temp_pjj->gn;
      *sval_quasi = 0.0;
      return;
    }
    else 
    {
      *which_piece = TRANSITION;
      *gval_quasi = temp_pjj->glarge;
      *sval_quasi = val_quasi * (temp_pjj->gn - temp_pjj->glarge);
      return;
    }
  }
...
void
pjj_fix_trap(int source_only, int *need_lu, double *hptr)
{
  dev_pjj *temp;
  device *temp_dev;
  mod_pjj *temp_moddata;
  ...
  *need_lu = FALSE;
  ...
  while (temp_dev != NULL)
  {
    ...
    temp_moddata = (mod_pjj *) (temp->mod->data);
    ic = temp->ic;
    if (temp->con_dev != NULL)
    if (temp_moddata->cct == SINXX)
    {
      icon = pjj_condev_i(temp, last_time + *hptr);
    ...
    pjj_quasi(temp, myabs(temp->vn_guess), &current_piece, &current_g,
             &source_val);
 ...
 */
    ...
    source_val = ic*sin(phi_guess-PI) + source_val;
...
update_pjj()
{
  ...
  while (temp_dev != NULL)
  {
    ...
    xnd = x_unk_copy[temp_pjj->n_plus]->yn;

    yn = x_unk_copy[temp_pjj->n_minus]->xn;
    ynd = x_unk_copy[temp_pjj->n_minus]->yn;

    temp_pjj->poly_coeff[0] = xn - yn;
    temp_pjj->poly_coeff[1] = (xnd - ynd);

    temp_pjj->ic_past = temp_pjj->ic_current;
...
double
pjj_step_limit(double current_step)
{
  device *temp_dev;
  dev_pjj *temp_pjj;
  double smallest;
  ...
  while (temp_dev != NULL)
  {
    ...
    if (temp_dev->type == P_JJ)
    { 
      delta_phi = myabs(x_unknown[temp_pjj->n_phi]->xn -
                *(x_unknown[temp_pjj->n_phi]->xpast));
    

      delta_phi = delta_phi*VN_GUESS_REF/
                (vn_guess_min + myabs(temp_pjj->vn_guess));
...
int
pjj_apriori_step_limit(double step, double *newstep)
{
  int i;
  dev_pjj *temp;
  device *temp_dev;
...
}  /* pjj_apriori_step_limit */


void
pjj_tran_print(FILE *fp, double inc_rate, int prtype, int subtype,
    device *temp_dev)
{
  long plus, minus, phi; 
  double pjj_area, i_joseph, i_cap, i_resis, i_total, j_phi;
  double plus_val, minus_val, phi_val, pd_val, md_val;
  modeldata *mod;

  plus = ((dev_pjj *) temp_dev->data)->n_plus;
  minus = ((dev_pjj *) temp_dev->data)->n_minus;
  phi = ((dev_pjj *) temp_dev->data)->n_phi;
  pjj_area = ((dev_pjj *) temp_dev->data)->area;
  mod = ((dev_pjj *) temp_dev->data)->mod;
  ...
  if (prtype == AMP)
  {
    ...
    i_joseph = get_pr_pjjic(inc_rate, (dev_pjj *) temp_dev->data) *
                 sin(phi_val);
    i_cap = (((dev_pjj *) temp_dev->data)->cap)* (pd_val - md_val);
    i_resis = pjj_area*jjiv(plus_val - minus_val, mod);  /* jjiv()*/
